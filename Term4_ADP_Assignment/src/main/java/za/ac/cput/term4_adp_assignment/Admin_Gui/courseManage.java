...
/**
 * Administrator course management interface for managing course catalog.
 * Provides functionality to view, add, edit, and delete courses with real-time updates.
 */
public class courseManage extends reusable implements ActionListener {
    ...
    /**
     * Constructs the course management interface with statistics and course table.
     * Initializes all GUI components and sets up real-time data updates.
     */
    public courseManage() {
        currentCourseData = new Object[0][];
        ...
        // Sets panel for and imports Application logo
        logo = new JPanel();
        ...
        // Administrator profile section along with admin icon
        ...
        // Active Course Management menu item 
        ...
        studs2.setBackground(DARK_GREEN);
        ...
        // Enrollment Management menu items
        ...
        // Bar for the course management title 
    ...
    /**
     * Fetches total course count from server and returns the total number of courses in the system.
     */
    ...
    /**
     * Starts automatic refresh timers for statistics and course table.
     */
    public void startAutoRefresh() {
        ...
        // Refresh course table every 4 seconds for real-time updates
        Timer tableTimer = new Timer(4000, e -> refreshCourseTable());
        tableTimer.start();
    }

    /**
     * Creates the initial course table.
     */
    ...
    /**
     * Fetches course data from server.
     */
    private Object[][] getCourseFromServer() {
        java.util.List<Object[]> courses = new java.util.ArrayList<>();
        try {
            String response = Client.sendRequest("GET_ALL_COURSES");
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error loading course data from server.");
        }
    ...
    /**
     * Populates the combo box with course selections.
     */
    private void populateComboBox() {
        com.removeAllItems(); // Clear existing items

        com.addItem("Select a Course");

        for (Object[] course : currentCourseData) {
            String displayText = course[0] + " - " + course[1]; 
            com.addItem(displayText);
        }
    }

    /**
     * Refreshes the course table with current data from server.
     */
    private void refreshCourseTable() {
        ...
        Object[][] newCourseData = getCourseFromServer();

        // Convert data to simple string for comparison
        String currentData = java.util.Arrays.deepToString(newCourseData);
        ...
        if (currentData.equals(lastStudentData)) {
            ...
            String[] columnNames = {
                "Course Code", "Course Name", "Credits", "Type", "Description", "Enrolled Students"
            };

            // Creates new table model with updated data
            javax.swing.table.DefaultTableModel model = new javax.swing.table.DefaultTableModel(currentCourseData, columnNames) {
                @Override
            ...
            courseTable.setBackground(BG_SURFACE);
            courseTable.setForeground(TEXT_PRIMARY);
            courseTable.setFont(new Font("Segoe UI", Font.PLAIN, 12));
            courseTable.setRowHeight(35);
            courseTable.getTableHeader().setBackground(BG_SURFACE);
            courseTable.getTableHeader().setForeground(DARK_GREEN);
    ...
    /**
     * Handles addition of new course with duplicate validation.
     */
    private void handleAddCourse() {
        // Show the input dialogs to get course details
        String courseId = JOptionPane.showInputDialog(this, "Enter Course Code:", "Add Course", JOptionPane.PLAIN_MESSAGE);
        ...
        try {
            String checkResponse = Client.sendRequest("CHECK_COURSE_EXISTS," + courseId);
            if ("EXISTS".equals(checkResponse)) {
                JOptionPane.showMessageDialog(this, "Course code '" + courseId + "' already exists!\nPlease use a different course code.", "Duplicate Course Code", JOptionPane.ERROR_MESSAGE);
                return;
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error checking course code availability.");
            return;
        }

        String name = JOptionPane.showInputDialog(this, "Enter Course Name:", "Add Course", JOptionPane.PLAIN_MESSAGE);
        if (name == null || name.trim().isEmpty()) {
            return;
        }

        String creditsStr = JOptionPane.showInputDialog(this, "Enter Credits:", "Add Course", JOptionPane.PLAIN_MESSAGE);
        if (creditsStr == null || creditsStr.trim().isEmpty()) {
            return;
        }

        String[] types = {"Core", "Elective"};
        String type = (String) JOptionPane.showInputDialog(this, "Select Course Type:", "Add Course",
                JOptionPane.PLAIN_MESSAGE, null, types, types[0]);
        if (type == null) {
            return;
        }

        String description = JOptionPane.showInputDialog(this, "Enter Course Description:", "Add Course", JOptionPane.PLAIN_MESSAGE);
        if (description == null) {
            description = "";
        }
        ...
        try {
            ...
            String response = Client.sendRequest("ADD_COURSE," + courseId + "," + name + "," + credits + "," + type + "," + description);
    ...
    /**
     * Handles editing of existing course information.
     */
    private void handleEditCourse() {
        ...
        Object[] courseToEdit = null;
        for (Object[] course : currentCourseData) {
            if (course[0].toString().equals(courseCode)) {
                courseToEdit = course;
                break;
            }
        }
        ...
        if (courseToEdit != null) {
            String currentName = courseToEdit[1].toString();
            String currentCredits = courseToEdit[2].toString();
            String currentType = courseToEdit[3].toString();
            String currentDescription = courseToEdit[4].toString();

            // replaces the name with current value as default
            String newName = (String) JOptionPane.showInputDialog(this,
                    "Enter new course name:", "Edit Course",
                    JOptionPane.PLAIN_MESSAGE, null, null, currentName);
            if (newName == null || newName.trim().isEmpty()) {
                return;
            }

            // replaces the credits with current value as default
            String newCreditsStr = (String) JOptionPane.showInputDialog(this,
                    "Enter new credits:", "Edit Course",
                    JOptionPane.PLAIN_MESSAGE, null, null, currentCredits);
            if (newCreditsStr == null || newCreditsStr.trim().isEmpty()) {
                return;
            }

            // replaces the type with current value as default
            String[] types = {"Core", "Elective"};
            String newType = (String) JOptionPane.showInputDialog(this,
                    "Select course type:", "Edit Course",
                    JOptionPane.PLAIN_MESSAGE, null, types, currentType);
            if (newType == null) {
                return;
            }

            // replaces description with current value as default
            String newDescription = (String) JOptionPane.showInputDialog(this,
                    "Enter new description:", "Edit Course",
            ...
            try {
                ...
                String response = Client.sendRequest("UPDATE_COURSE," + courseCode + "," + newName + "," + newCredits + "," + newType + "," + newDescription);
                if ("SUCCESS".equals(response)) {
    ...
    /**
     * Handles deletion of course with confirmation and cascade deletion of enrollments.
     */
    private void handleDeleteCourse() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                String response = Client.sendRequest("DELETE_COURSE," + courseCode);
                if ("SUCCESS".equals(response)) {
